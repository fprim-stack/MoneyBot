...
def update_user_data(user_id, money=None, xp=None, level=None, inventory=None, last_daily=None, bank=None, achievements=None):
    """Update user data"""
    users = load_users()
    user_id = str(user_id)
    
    if user_id not in users:
        users[user_id] = {"money": 100, "xp": 0, "level": 1, "inventory": {}, "last_daily": 0, "bank": 0, "achievements": []}
...
async def gamble(ctx, amount: int):
    ...
    if won:
        # Double the money
        winnings = amount * 2
        new_balance = add_money(ctx.author.id, amount)  # They get their bet back plus winnings
        embed = discord.Embed(
            title="🎉 You Won!",
...
async def cards(ctx, bet: int = 100):
    ...
    def card_value(cards):
        ...
        aces = 0
        for card in cards:
            rank = card[:-2] if card[:-2] in ['10'] else card[:-1]
            if rank in ['J', 'Q', 'K']:
                value += 10
            elif rank == 'A':
                aces += 1
...
async def roll(ctx):
    ...
    # 0.0000000000000000000000000000000001% chance for Fragment Of Reality (NULL rarity)
    if ultra_rare_chance < 0.0000000000000000000000000000000001:
        ...
        embed = discord.Embed(
            ...
            color=discord.Color.from_rgb(0, 0, 0)  # Pure black
        )
    ...
    # 0.00000001% chance for Cosmic items
    elif ultra_rare_chance < 0.0000000001:
        cosmic_items = [
        ...
        # Normal weighted selection for common through legendary
        # Filter out cosmic and null items for normal rolling
        normal_items = {
        ...
        # Create weighted list
        weighted_items = []
        for item_name, item_data in normal_items.items():
            rarity = item_data.get("rarity", "common")
            weight = rarity_weights.get(rarity, 1)
            weighted_items.extend([item_name] * weight)
        
        # Roll for item
        rolled_item = random.choice(weighted_items)
        item_data = items_db[rolled_item]
        rarity = item_data.get("rarity", "common")
        value = item_data.get("value", 0)
